...
IBR_m = [ [1/(2*SG(1).H)*IBR(1).ssr.C ; zeros(5,ns_ibr)] [zeros(2,ns_ibr); 1/(2*SG(3).H)*IBR(2).ssr.C; zeros(3,ns_ibr)] [ zeros(4,ns_ibr);  1/(2*SG(5).H)*IBR(3).ssr.C; zeros(1,ns_ibr)]];

Grid.ssr.A = [KD_m                          L_m                 IBR_m;
              eye(nbus)                     zeros(nbus, nbus)   zeros(nbus, nibrs*ns_ibr);
              zeros(nibrs*ns_ibr, 2*nbus)   blkdiag(IBR(1).ssr.A, IBR(2).ssr.A, IBR(3).ssr.A)];
...
IBR_Bm  = [zeros(nbus, nibrs*nu_ibr); blkdiag(IBR(1).ssr.B, IBR(2).ssr.B,  IBR(3).ssr.B )];
